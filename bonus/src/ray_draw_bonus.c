...
void    init_values(t_game *g, t_raycast *ray, int x)
{
...
void draw_textr(t_game *g, t_raycast *ray, int x)
{
    ...
    {
        ray->tex_y = (int)tex_pos % g->img.height;
        tex_pos += step;

        if (is_door)
        {
            if (i == ray->draw_start || i == ray->draw_end - 1 || 
                i < ray->draw_start + border_size || i > ray->draw_end - border_size - 1)
                color = 0x1E90FF;
            else
                color = 0x00008B;
        }
